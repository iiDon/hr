...
const JobsChart = () => {
  ...
  useEffect(() => {
    jobs.map((job) => {
      if (new Date(job.created!).getMonth() === new Date().getMonth() - 1) {
        setMonthOne((prev) => prev + 1);
      }
      if (new Date(job.created!).getMonth() === new Date().getMonth() - 2) {
        setMonthTow((prev) => prev + 1);
      }
      if (new Date(job.created!).getMonth() === new Date().getMonth() - 3) {
        setMonthThree((prev) => prev + 1);
      }
      if (new Date(job.created!).getMonth() === new Date().getMonth() - 4) {
